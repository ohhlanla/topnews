...
class mvp_facebook_widget extends WP_Widget {
	...
	function mvp_facebook_widget() {
		...
		/* Create the widget. */
		$this->__construct( 'mvp_facebook_widget', __('Top News: Facebook Widget', 'mvp_facebook_widget'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		<iframe src="//www.facebook.com/plugins/likebox.php?href=<?php echo esc_url( $page_url ); ?>&amp;width=<?php echo esc_html( $width ); ?>&amp;height=<?php echo esc_html( $height ); ?>&amp;show_faces=<?php if($faces) { echo 'true'; } else { echo 'false'; } ?>&amp;colorscheme=light&amp;stream=<?php if($stream) { echo 'true'; } else { echo 'false'; } ?>&amp;show_border=true&amp;header=<?php if($header) { echo 'true'; } else { echo 'false'; } ?>" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:<?php echo esc_html( $width ); ?>; height:<?php echo esc_html( $height ); ?>px;" allowTransparency="true"></iframe>
